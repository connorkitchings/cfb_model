...
def validate_ingestion(year: int):
    ...
    if not games:
        ...
        required_cols = ["game_id", "week", "home_team", "away_team", "home_points", "away_points"]
        ...
        if missing:
             ...
             if df_games[required_cols].isnull().any().any():
                 ...
                 errors.append(f"❌ [Games] Null values detected in required columns:\n{null_counts[null_counts > 0]}")
    ...
    if not betting:
        ...
        if "spread" in df_bet.columns:
            ...
            if not outliers.empty:
                errors.append(f"⚠️ [Betting] {len(outliers)} spread outliers detected (<-70 or >70)")
