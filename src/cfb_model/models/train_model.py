...
def main(cfg: DictConfig) -> None:
    ...
    train_years = [int(y.strip()) for y in cfg.train_years.split(",") if y.strip()]
    ...
    out_dir = os.path.join(cfg.model_dir, str(test_year))
    ...
    with mlflow.start_run(run_name=f"Ensemble_Training_{test_year}") as parent_run:
        ...
        for name, model in spread_models.items():
            with mlflow.start_run(run_name=f"spread_{name}", nested=True):
                ...
                joblib.dump(model, os.path.join(out_dir, f"spread_{name}.joblib"))
        ...
        for name, model in total_models.items():
            with mlflow.start_run(run_name=f"total_{name}", nested=True):
                ...
                joblib.dump(model, os.path.join(out_dir, f"total_{name}.joblib"))
                mlflow.sklearn.log_model(model, f"total_{name}")

        # --- Evaluate and Log Ensemble Performance ---
        print("Evaluating ensemble predictions...")
        spread_preds = [joblib.load(os.path.join(out_dir, f"spread_{name}.joblib")).predict(X_test) for name in spread_models]
        total_preds = [joblib.load(os.path.join(out_dir, f"total_{name}.joblib")).predict(X_test) for name in total_models]
        ...
        metrics_path = os.path.join(cfg.metrics_dir, f"model_eval_{test_year}.csv")
        pd.DataFrame(
            [
                {
