...
## 2025-10-24 — XGBoost Hyperparameter Tuning for Points-For Model

- Category: Modeling / Hyperparameter Tuning
- Decision: Tuned hyperparameters for the XGBoost model for the points-for prediction task.
- Rationale: To optimize the performance of the points-for model, particularly for totals predictions, by finding the best set of hyperparameters.
- Impact: The `conf/model/points_for_xgboost.yaml` file was updated with the optimal hyperparameters (n_estimators: 150, learning_rate: 0.05, max_depth: 5, subsample: 0.9, colsample_bytree: 0.9). Walk-forward validation was performed to benchmark its performance against the legacy ensemble.
- Next Steps: Analyze the `metrics_summary.csv` to compare the performance of the tuned XGBoost "points-for" model against the legacy ensemble, especially for "totals" predictions. Based on this analysis, decide whether to adopt the XGBoost model for totals.
- References: `scripts/optimize_hyperparameters.py`, `conf/model/points_for_xgboost.yaml`, `artifacts/validation/walk_forward/metrics_summary.csv`, `session_logs/2025-10-24/01.md`
...
- Impact:
  ...
  - Refactored `src/models/train_model.py` to use nested MLflow runs for tracking the training of the model ensemble.
  - This sets the foundation for subsequent integration of Hydra and Optuna.
- References: `docs/guides/MLOps_Integration_Guide.md`, `src/models/train_model.py`
...
- References: `src/data/aggregations/core.py`, `reports/2024/weekly_hit_summary_2024.csv`
...
- Impact: `src/data/ingestion/betting_lines.py` modified. `scripts/publish_picks.py` modified.
...
- Impact:
  - `src/models/train_model.py` was reverted to use `build_feature_list` for all models.
...
- References: `scripts/run_feature_selection.py`, `src/models/train_model.py`, `[LOG:2025-10-07/01]`
...
- References: `src/scripts/generate_weekly_bets_clean.py`, `scripts/simulate_bankroll_2024.py`, `scripts/run_weekly_reports_2024.py`, `[LOG:2025-10-02]`
...
- References: `src/scripts/generate_weekly_bets_clean.py`, `reports/2025/CFB_week{WW}_bets.csv`, session log `[LOG:2025-10-02]`
...
- References: `src/scripts/generate_weekly_bets_clean.py` (try/except around SHAP with predict fallback), `[LOG:2025-10-02]`
...
- Impact: `src/data/storage/local_storage.py` and `src/data/aggregations/persist.py` updated; docs and scripts remain compatible.
...
- Impact:
  - `src/models/train_model.py` was updated to train and save a collection of models for each target.
  - `src/scripts/generate_weekly_bets_clean.py` was updated to load all models for a target, generate predictions from each, and average the results.
...
- References: `src/models/train_model.py`, `src/scripts/generate_weekly_bets_clean.py`, `[LOG:2025-09-30]`
...
- Impact: Model training pipeline (`src/models/train_model.py`) updated to use RandomForest for totals. Both models saved to `artifacts/models/<year>/spread_model.joblib` and `artifacts/models/<year>/total_model.joblib`. Weekly prediction script continues to work unchanged as it loads models dynamically.
...
- References: `src/models/train_model.py`, `scripts/model_improvement_experiments.py`, session log `[LOG:2025-09-30]`
...
- Decision:
  ...
  4.  **Refactor Legacy Code:** Remove the redundant `src/models/train_model.py` script and fix the corresponding import test.
...
- References: `scripts/cache_weekly_stats.py`, `scripts/cli.py`, `src/scripts/generate_weekly_bets_clean.py`, `[LOG:2025-09-29]`
...
- References: `scripts/run_full_season.py`, `src/scripts/generate_weekly_bets_clean.py`, `reports/2024/edge_threshold_sweep_total.csv`, [LOG:2025-09-28]
...
- References: `src/scripts/generate_weekly_bets_clean.py`, `scripts/score_weekly_picks.py`, [LOG:2025-09-25]
...
- References: `src/models/features.py`, `src/models/ridge_baseline/train.py`, `src/scripts/generate_weekly_bets_clean.py`, [LOG:2025-09-24]
...
- References: `src/data/storage/local_storage.py`, [LOG:2025-09-04]
...
- References: `src/data/validation.py`, [LOG:2025-09-04]
...
- References: `scripts/aggregations_cli.py`, `src/data/aggregations/persist.py`, [LOG:2025-09-04]
...
- References: `src/data/validation.py` (validate_team_game_vs_boxscore), [LOG:2025-09-05]
