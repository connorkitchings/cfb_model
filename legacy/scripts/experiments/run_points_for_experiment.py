...
def main(cfg: DictConfig):
    ...
    if home_params_path.exists():
        log.info(f"Loading optimized home params from {home_params_path}")
        with open(home_params_path, "r") as f:
            # Parse YAML manually or use OmegaConf/PyYAML
            # The file format is simple key: value
            import yaml

            home_params = yaml.safe_load(f)
    else:
        home_params = cfg.model.params

    if away_params_path.exists():
        log.info(f"Loading optimized away params from {away_params_path}")
        with open(away_params_path, "r") as f:
            import yaml

            away_params = yaml.safe_load(f)
    else:
        away_params = cfg.model.params
